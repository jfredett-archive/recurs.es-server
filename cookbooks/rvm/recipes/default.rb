...
%w(gcc patch curl zlib readline libxml2 libxslt git autoconf automake diffutils
  make libtool bison subversion).
  each do |pkg|
    package pkg do
      action :install
    end
...
# make the rvm-installer user an rvm-approved user
rvm_user 'rvm'
